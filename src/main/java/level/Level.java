...
public class Level
{
    ...
    private boolean isPlayerInControl = true;
    ...
    public void update()
    {
        ...
        if ( isPlayerInControl && collision() )
        {
            bird.fall();
            isPlayerInControl = false;
        }

        if ( !isPlayerInControl && Input.isKeyDown( GLFW_KEY_SPACE ) )
            reset = true;
    ...
    private boolean collision()
    {
        for ( int i = 0; i < 5 * 2; i++ )
        {
            ...
            if ( bx1 > px0 && bx0 < px1 ) // right side of the bird
            {
                if ( by1 > py0 && by0 < py1 )
                {
                    return true;
